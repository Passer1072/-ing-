...
void chaRuPaiXu() {
    ...
    for (int i = 1; i < MaxStu; ++i) {              // 外循环遍历二维表
        temp1 = passer[i][1];                       // 储存成绩
        temp2 = passer[i][0];                       // 存储学号
        j = i - 1;                                  // 表示在已排序的部分中的前一个元素的索引。

        while (j >= 0 && passer[j][1] > temp1) {    //
            passer[j + 1][1] = passer[j][1];        // 将已排序部分中的较大成绩向下移动一位，为 temp 腾出位置
            passer[j + 1][0] = passer[j][0];
            j = j - 1;                               // 值递减，以便在已排序部分中继续向上比较
        }

        passer[j + 1][1] = temp1;                   //  当 while 循环结束时，j 已经找到了 temp 应该插入的位置，这一行将 temp 的值插入到正确的位置，完成了插入操作。
        passer[j + 1][0] = temp2;
    }
}         // 插入排序
